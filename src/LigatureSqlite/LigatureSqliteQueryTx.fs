...
let readValue (rd: IDataReader): Result<Value, LigatureError> =
    let valueIdentifier = rd.ReadStringOption "value_identifier" |> Option.map identifier
...
type LigatureSqliteQueryTx(dataset: Dataset, datasetId: int64, conn, tx) =
    ...
    let createValueParams (value: Value): Result<(string * SqlType) list, LigatureError> =
        ...
        | Identifier(value) ->
            ...
            match id with
            | Ok(Some(id)) -> Ok ["value_identifier", SqlType.Int64 id; "value_string", SqlType.Null; "value_integer", SqlType.Null]
            | Ok(None) -> todo
            | Error(err) -> Error(err)
        | String(value) ->
    ...
    interface QueryTx with
        ...
        member _.MatchStatements entity attribute value =
            ...
            let queryCondition: string =
                ...
                |> List.fold (fun query nextElement ->
                    query + 
                        if nextElement = "" then
                            nextElement
                        else
