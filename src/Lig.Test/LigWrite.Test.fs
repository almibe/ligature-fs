...
[<Tests>]
let tests =
  testList "Lig Write Suite" [
    testCase "Write Identifier" <| fun _ ->
        Expect.equal (writeIdentifier(unwrap(identifier("hello")))) "<hello>" ""
    testCase "Write Value" <| fun _ ->
        Expect.equal (writeValue(Identifier(unwrap(identifier("hello"))))) "<hello>" "" // identifier
