...
[<Tests>]
let tests =
    testList "Lig Read Suite" [
        testCase "read Identifiers" <| fun _ ->
            Expect.equal (readIdentifier (Gaze.fromString "<a>")) (identifier "a") ""

        testCase "read Values" <| fun _ ->
            Expect.equal (readValue (Gaze.fromString "<a>")) (Ok(Identifier(unwrap(identifier "a")))) ""
            //TODO fix below
        ...
        testCase "read single Statement" <| fun _ ->
            Expect.equal (readLig "<a> <b> <c>") (Ok([(statement "a" "b" (Identifier(unwrap(identifier("c")))))])) ""
    ]
