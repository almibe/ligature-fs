...
module Views =
    ...
    let reportError (error: LigatureError) = printfn $"{error.userMessage}"
    ...
    let addDatasetForm (state: IWritable<UIModel>) =
        ...
        StackPanel.create
            [ StackPanel.children
                  [ TextBlock.create [ TextBlock.text "Dataset Name:" ]
                    TextBox.create
                        [ TextBox.onTextChanged (fun newDataset ->
                              state.Set
                                  { state.Current with
                                      NewDataset = newDataset }) ]
                    Button.create [ Button.content "Add"; Button.onClick (fun _ -> addDataset ()) ] ] ]
    ...
    let removeDatasetForm (state: IWritable<UIModel>) =
        ...
        StackPanel.create
            [ StackPanel.children
                  [ TextBlock.create [ TextBlock.text "Remove?" ]
                    Button.create [ Button.content "Yes"; Button.onClick (fun _ -> removeDataset ()) ] ] ]
              ...
              StackPanel.children
                  [ TextBlock.create
                    ...
                    ComboBox.create
                        [ ComboBox.verticalAlignment VerticalAlignment.Center
                          ComboBox.margin (5, 0, 10, 0)
                          ComboBox.dataItems [ "Query"; "Add Statements"; "Remove Statements" ]
                          ComboBox.selectedIndex 0 ]
                    Button.create
                        [ Button.verticalAlignment VerticalAlignment.Center
                          Button.margin (10, 0, 10, 0)
    ...
    let topControls (state: IWritable<UIModel>) =
        StackPanel.create
            [ StackPanel.orientation Orientation.Vertical
              ...
              StackPanel.children [ datasets state; actions state ] ]

    let editor (state: IWritable<UIModel>) =
        Grid.create
            [ Grid.rowDefinitions "*, 1, *"
              Grid.children
                  [ TextBox.create [ Grid.row 0; TextBox.acceptsReturn true; TextBox.classes [ "code" ] ]
                    GridSplitter.create [ Grid.row 1; GridSplitter.background "Gray" ]
                    TextBox.create
                        [ Grid.row 2
    ...
    let main () =
        ...
        Component(fun (ctx: IComponentContext) ->
            ...
            DockPanel.create [ DockPanel.children [ topControls state; editor state ] ])
