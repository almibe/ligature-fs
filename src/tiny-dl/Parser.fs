...
let conceptConjunctionNib (gaze: Gaze.Gaze<Token>) : Result<ConceptExpression, Gaze.GazeError> =
    failwith "TODO"
    // match (Gaze.next gaze, Gaze.next gaze, Gaze.next gaze) with
...
let conceptInclusionNib (gaze: Gaze.Gaze<Token>) : Result<KnowledgeBase, Gaze.GazeError> =
    ...
    //     | Ok(Token.Comma) | Error Gaze.NoMatch ->
    //         Ok(Set.ofList [ Inclusion { left = concept; right = AtomicConcept concept' } ], emptyABox)
    //     | _ ->
    //     // Ok(
    //     //     Set.ofList
    //     //         [ Inclusion
...
let conceptExpressionNib =
    takeFirst
        [ atomicConceptNib
          unaryConceptExpressionNib ]
