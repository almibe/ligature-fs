...
let tests =
    testList
        ...
        [ testCase "Run Integer"
          ...
          <| fun _ ->
              Expect.equal (run "()" bindings) (Ok(Tuple([]))) "Empty tuple"
              Expect.equal (run "(true false 1 2 \"hello\")" bindings) (Ok(Tuple([Boolean(true); Boolean(false); Integer(1); Integer(2); String("hello")]))) "Tuple with Values"
              Expect.equal (run "( if true false else true )" bindings) (Ok(Tuple([Boolean(false)]))) "Tuple with Conditional"
              Expect.equal (run "( { let x = 5 x} 5 6)" bindings) (Ok(Tuple([Integer(5); Integer(5); Integer(6)]))) ""
