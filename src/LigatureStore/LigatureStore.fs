...
type LigatureStore(path: string option) =
    ...
    member _.Init() =
        ...
        while cont do
            ...
            if succ then
                ...
                | 2, name, e, r, v -> 
                    store.AssertKnowledgeBase name (Set.ofList [Term e, Term r, Term v])
                | 3, name, e, r, v ->
                    store.DefineKnowledgeBase name (Set.ofList [Term e, Term r, Term v])
                | 4, name, e, r, v ->
                    store.UnassertKnowledgeBase name (Set.ofList [Term e, Term r, Term v])
                | 5, name, e, r, v ->
                    store.UndefineKnowledgeBase name (Set.ofList [Term e, Term r, Term v])
    ...
    interface ILigatureStore with
        member this.KnowledgeBases(): string seq = 
        ...
        member this.AssertKnowledgeBase(name: string) (network: Network): unit = 
            ...
            Set.iter (fun (Term e, Term r, Term v) -> 
                let event = e_assert, name, e, r, v
                let encoded = generator.Encode event
                log.Enqueue encoded |> ignore) network
            log.Commit()
